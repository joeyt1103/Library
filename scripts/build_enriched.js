...
async function enrichOne(book) {
  ...
  return { title, author, isbn, ...out };
}

async function main() {
  const raw = JSON.parse(await fs.readFile(INPUT, "utf-8"));

  // raw might already have Title/Author/ISBN keys depending on how you created it
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    if (i % 20 === 0) console.log(`Enriched ${i + 1}/${books.length}`);
